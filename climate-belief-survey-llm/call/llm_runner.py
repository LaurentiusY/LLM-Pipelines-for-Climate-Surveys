...
def process_one_task_two_stage(task_args, temp_file, client, cfg):
    ...
    处理单个任务（两阶段）：
      Stage1 -> plan_json
      Stage2 -> final_json (scored)

    task_args: (row_id, repeat_idx, task_type, task_obj, seed_base, plan_token)
    """
...
def reconstruct_from_jsonl(temp_file):
    """从JSONL临时文件重建最终数据结构（只用 stage2 的 data）"""
    results_flat = []
    ...
    for item in results_flat:
        ...
        if task_type == 'belief':
            target['belief'] = data
            target['plans']['belief'] = plan
        elif task_type == 'policy':
            target['policy'] = data
            target['plans']['policy'] = plan
        elif task_type == 'share':
            if isinstance(data, dict):
                target['share'] = data.get('Q1')
            else:
                target['share'] = data
            target['plans']['share'] = plan
        elif task_type == 'trees':
            if isinstance(data, dict):
                target['trees'] = data.get('trees')
            else:
                target['trees'] = data
            target['plans']['trees'] = plan
...
def main(run_dir, cfg, prompts_data=None):
    """
    Run LLM calls for all tasks.

    Args:
        run_dir: Output directory for results.
        cfg: Configuration namespace.
        prompts_data: Dict returned by persona module, containing:
            row_ids, get_persona, build_prompts (lazy mode)
            — or legacy prompts.json dict with "samples" key.
    """
    ...
    # 3. 统计待处理任务总数
    task_types = ["belief", "policy", "share", "trees"]
    total_count = 0
    for rid in row_ids:
            ...
            with tqdm(total=total_count, unit="task") as pbar:
                ...
                for batch_start in range(0, len(row_ids), SAMPLE_BUFFER_SIZE):
                    ...
                    for rid in batch_rids:
                        ...
                        task_key_map = {"belief": "task_1", "policy": "task_2",
                                        "share": "task_3", "trees": "task_4"}
