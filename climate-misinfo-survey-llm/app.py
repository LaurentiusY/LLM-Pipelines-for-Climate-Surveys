"""
Terminal UI for Climate Misinfo Survey - LLM Pipeline.
...
def _display_results(run_dir: str):
    ...
    affect_sci = summary[(summary["Task"] == "Affect_Science") & (summary["Scale"] == "original")]
    affect_act = summary[(summary["Task"] == "Affect_Action") & (summary["Scale"] == "original")]
    ccb = summary[(summary["Task"] == "CCB") & (summary["Scale"] == "original")]

    if len(affect_sci) > 0:
        r = affect_sci.iloc[0]
        print(f"  Affect_Science  MAE={r['MAE']:.2f}  Mean_Error={r['Mean_Error']:.2f}  N={int(r['N'])}")
    if len(affect_act) > 0:
        r = affect_act.iloc[0]
        print(f"  Affect_Action   MAE={r['MAE']:.2f}  Mean_Error={r['Mean_Error']:.2f}  N={int(r['N'])}")
    if len(ccb) > 0:
        r = ccb.iloc[0]
        print(f"  CCB             MAE={r['MAE']:.2f}  Mean_Error={r['Mean_Error']:.2f}  N={int(r['N'])}")

    print("\n  ── Ternary Classification (Neg / Neutral / Pos) ──────────")
    for task_label, sheet in [("Affect_Science", "AffectSci_Ternary_Metrics"),
                               ("Affect_Action",  "AffectAct_Ternary_Metrics")]:
        ...
        if df is not None and len(df) > 0:
            ...
            n   = r.get("N",        float("nan"))
            print(f"  {task_label:<16}  Accuracy={acc:.4f}  F1={f1:.4f}  N={int(n)}")
        else:
            print(f"  {task_label:<16}  N/A")

    print("\n  ── Correlation (Pearson r / Spearman r) ──────────────────")
    for task_label, sheet in [("Affect_Science", "AffectSci_Correlation"),
                               ("Affect_Action",  "AffectAct_Correlation"),
                               ("CCB",            "CCB_Correlation")]:
        df = _load_sheet(run_dir, sheet)
        if df is not None and len(df) > 0:
    ...
    print("\n  ── Affect Delta (vs baseline Affect_T1_1) ────────────────")
    for task_label, sheet in [("Affect_Science", "AffectSci_Delta"),
                               ("Affect_Action",  "AffectAct_Delta")]:
        df = _load_sheet(run_dir, sheet)
        if df is not None and len(df) > 0:
            valid = df.dropna(subset=["true_delta", "pred_delta"])
            if len(valid) > 0:
                mae_delta   = (valid["pred_delta"] - valid["true_delta"]).abs().mean()
                mean_delta  = (valid["pred_delta"] - valid["true_delta"]).mean()
                print(f"  {task_label:<16}  Delta MAE={mae_delta:.2f}  Delta Mean_Error={mean_delta:.2f}  N={len(valid)}")
        else:
            print(f"  {task_label:<16}  N/A")

    print("\n  ── MIST (Truth Discernment) ───────────────────────────────")
    mist = summary[summary["Task"] == "MIST"]
    mist_sdt = summary[summary["Task"] == "MIST_SDT"]
    if len(mist) > 0:
        r = mist.iloc[0]
        print(f"  Classification  Accuracy={r['Accuracy']:.4f}  F1={r['F1']:.4f}  N={int(r['N'])}")
    if len(mist_sdt) > 0:
        r = mist_sdt.iloc[0]
        dp = r.get("d_prime", float("nan"))
        cb = r.get("c_bias",  float("nan"))
        hr = r.get("Hit_Rate", float("nan"))
        fa = r.get("False_Alarm_Rate", float("nan"))
        print(f"  SDT             d'={dp:.4f}  c={cb:.4f}  Hit={hr:.4f}  FA={fa:.4f}")
...
def action_run():
    ...
    max_tokens_s2 = _ask_int("Max tokens Stage 2  (final answer)", 250)
    ...
    target_condition = _ask_choice(
        "Target condition  (All=all participants / Control / Cognitive / Socioaffective)",
        ["All", "Control", "Cognitive", "Socioaffective"], "All")
...
def main():
    print()
    print("=" * 60)
    print("  Climate Misinfo Survey - LLM Pipeline")
    print("=" * 60)
