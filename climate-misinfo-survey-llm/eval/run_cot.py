import os
import sys
from eval.data_loading import load_data, extract_raw_data, extract_plans_cot, aggregate_data, extract_affect_delta
...
from eval.excel_output import (
    save_plans_to_excel_cot, save_raw_to_excel,
...
# Human responses: 1=said true, 0=said false
# T items are TRUE statements, F items are FALSE statements
...
def main(run_dir):
    ...
    df_plans = extract_plans_cot(results_data)
    save_plans_to_excel_cot(df_plans, output_plans)
    ...
    # Affect delta (change from baseline)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m eval.run_cot <run_dir>")
        sys.exit(1)
    main(sys.argv[1])
